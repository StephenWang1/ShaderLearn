...
Shader "Unlit/Parallax"
{
    Properties
    {
        ...
        _ParallaxStrength ("_ParallaxStrength",range(0,0.1)) = 0.05
    ...
    SubShader
    {
        Pass
        {
            ...
            float4 _ParallaxMap_ST; // 命名是固定的贴图名+后缀"_ST"，4个值前两个xy表示缩放，后两个zw表示偏移
            ...
            v2f vert(a2v v)
            {
                ...
                f.uv.xy = v.texcoord * _MainTex_ST.xy + _MainTex_ST.zw; // 贴图的纹理坐标
                f.uv.zw = v.texcoord * _NormalMap_ST.xy + _NormalMap_ST.zw; // 法线贴图的纹理坐标
                f.uv0 = v.texcoord * _Detail_ST.xy + _Detail_ST.zw;
                ...
                v.tangent.xyz = normalize(v.tangent);
                v.normal = normalize(v.normal);
                float3x3 objectToTangentSpace = float3x3(v.tangent.xyz, cross(v.normal, v.tangent.xyz) * v.tangent.w,
                                                         v.normal);
                f.tangentViewDir = mul(objectToTangentSpace, ObjSpaceViewDir(v.vertex));
            ...
            float2 ParallaxRayMarching(float2 uv, float2 viewDir)
            {
                float step = 10;
                float2 offsetUv = 0;
                float stepSize = 1 / step;
                float2 uvDelate = viewDir * (stepSize * _ParallaxStrength);
                float stepHeight = 1;
                float surfaceHeight = GetParallaxHeight(uv);
                for (int i = 1; i < step && stepHeight > surfaceHeight; i++)
                {
                    offsetUv -= uvDelate *0.5;
                    stepHeight -= stepSize * 0.5;
                    surfaceHeight = GetParallaxHeight(uv + offsetUv);
                }
                return offsetUv;
            }

            void ApplyParallax(inout v2f f)
            {
                f.tangentViewDir = normalize(f.tangentViewDir);
                #define PARALLAX_BIAS 0.42
                f.tangentViewDir.xy /= (f.tangentViewDir.z + PARALLAX_BIAS);
                #if !defined(PARALLAX_FUNCTION)
                #define  PARALLAX_FUNCTION ParallaxRayMarching
                #endif

                float2 offsetUv = PARALLAX_FUNCTION(f.uv.xy, f.tangentViewDir.xy);
                f.uv.xy += offsetUv;
                f.uv.zw += offsetUv;
                f.uv0 += offsetUv * (_Detail_ST.xy / _MainTex_ST.xy);
            }

            // 要把所有跟法线方向有关的运算，都放到切线空间下。因为从法线贴图中取得的法线方向是在切线空间下的。
            fixed4 frag(v2f f) : SV_Target
            {
                f.tangentViewDir.xy /= (f.tangentViewDir.z + 0.42);
                ApplyParallax(f);
                /*float height = GetParallaxHeight(f.uv.xy);
                height -= 0.5;
                _ParallaxStrength *= height;
                f.tangentViewDir = normalize(f.tangentViewDir);*/
                //f.uv.xy +=f.tangentViewDir.xy*_ParallaxStrength;
                //f.uv.zw += f.tangentViewDir.xy*_ParallaxStrength;
                /*float2 offsetUv = f.tangentViewDir.xy * height;
                f.uv.zw += offsetUv;
                f.uv.xy += offsetUv * (_Detail_ST.xy/_MainTex_ST.xy);*/
                ...
                fixed3 detailColor = tex2D(_Detail, f.uv0);
                // 最终颜色 = 漫反射 + 环境光 
                fixed3 tempColor = diffuse + ambient * texColor * detailColor; // 让环境光也跟纹理颜色做融合，防止环境光使得纹理效果看起来朦胧
