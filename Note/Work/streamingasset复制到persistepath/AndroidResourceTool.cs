...
public class AndroidResourceTool : AssetPostprocessor, IEditorTool
{
    ...
    private void CopyLuaRes(string targetDir)
    {
       ...
       // string targetDir = CSEditorPath.Get(EEditorPath.StreamingAssetsPath) + "/luaRes";
        ...
        RecordFilesName(targetDir);
    }

    private void RecordFilesName(string filePath)
    {
        var files= Directory.GetFiles(filePath, "*.lua", SearchOption.AllDirectories);
        CSStringBuilder.Clear();
        for (int i = 0; i < files.Length; i++)
        {
            var relatePath = files[i];
            var index = relatePath.IndexOf("luaRes");
            if(index>0)
                relatePath = relatePath.Substring(index);
            else
            {
                Debug.Log("error :"+files[i]);
            }
           // relatePath = relatePath.Replace('\\', '/');
            CSStringBuilder.Append(relatePath + "\r\n");
        }
        var filesNamePath = $"{filePath}/luaResPath.txt";
        var data = Encoding.UTF8.GetBytes(CSStringBuilder.ToString());
        File.WriteAllBytes(filesNamePath,data);
    }
