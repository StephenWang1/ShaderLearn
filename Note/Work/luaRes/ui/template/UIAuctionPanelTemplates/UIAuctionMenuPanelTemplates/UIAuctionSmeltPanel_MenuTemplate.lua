---@class UIAuctionSmeltPanel_MenuTemplate:UIAuctionMenuPanelTemplateBase
local UIAuctionSmeltPanel_MenuTemplate = {}

setmetatable(UIAuctionSmeltPanel_MenuTemplate, luaComponentTemplates.UIAuctionMenuPanelTemplateBase)

function UIAuctionSmeltPanel_MenuTemplate:GetFirstMenuData()
    if self.mFirstMenuData == nil then
        ...
        local res, info = CS.Cfg_GlobalTableManager.Instance.dic:TryGetValue(22516)
        if res then
            ...
            for i = 1, #strs do
                ...
                if #data > 0 then
                    --一级id
                    local firstId = tonumber(data[1])
                    table.insert(self.mFirstMenuData, firstId)
                    --二级id
                    for i = 2, #data do
                        local secondMenu = self.mFirstMenuIdToSecondMenu[firstId]
                        if secondMenu == nil then
                            secondMenu = {}
                            self.mFirstMenuIdToSecondMenu[firstId] = secondMenu
                        end
                        table.insert(secondMenu, tonumber(data[i]))
                    end
                end
            end
        end
...
---选中目录改变（用于发送请求）
function UIAuctionSmeltPanel_MenuTemplate:ChooseMenuChange()
    local data = {}
    ...
    luaEventManager.DoCallback(LuaCEvent.AuctionSmeltMenuChange, data)
end

---是否显示三级目录
function UIAuctionSmeltPanel_MenuTemplate:NeedShowThirdMenu()
    return true
end

function UIAuctionSmeltPanel_MenuTemplate:RefreshSecondMenu(secondMenuId)
...
--region 界面适配
---刷新目录位置
function UIAuctionSmeltPanel_MenuTemplate:SuitMenu()
...
return UIAuctionSmeltPanel_MenuTemplate