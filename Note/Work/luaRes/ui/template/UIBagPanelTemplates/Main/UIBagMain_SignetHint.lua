---印记背包面板，不筛选，对应的物品高亮显示，点击对应物品提示镶嵌按钮
---@class UIBagMainRole:UIBagMain_Normal
local UIBagMain_SignetHint = {}
setmetatable(UIBagMain_SignetHint, luaComponentTemplates.UIBagMainNormal)

---@return CSBagItemHint
function UIBagMain_SignetHint:GetBagItemHint()
...
--region 重写刷新方法
---刷新所有格子之前,获取推荐的装备队列
function UIBagMain_SignetHint:BeforeRefreshAllGrids()
    self.mList = self:GetBagItemHint():GetHintList(LuaEnumMainHint_BetterBagItemType.BetterSignet)
end

---刷新单个格子时,高亮显示提示的装备
function UIBagMain_SignetHint:RefreshSingleGrid(bagGrid, bagItemInfo, itemTbl)
...
--region 重写交互方法
---正常点击时,打开物品信息界面
function UIBagMain_SignetHint:OnGridClicked(bagGrid, bagItemInfo, itemTbl)
    if self.mList ~= nil then
        for i = 0, self.mList.Count - 1 do
            local temp = self.mList[i]
            if temp and bagItemInfo.lid == temp then
