...
function UIItemInfoPanel_ElementBaseAttribute:RefreshAttributes(bagItemInfo, itemInfo)
    ...
    if elementTab ~= nil then
        local elementAttackName = Utility.GetElementColor() .. CS.CSScene.MainPlayerInfo.ElementInfo:GetElementAttackName(elementTab)
    ...
    --region 套装等级
    if elementTab ~= nil then
        local titleName = CS.Cfg_GlobalTableManager.Instance:GetElementSuitName(elementTab.subType)
