---@class UIRankServantFeatureTemplate:UIRankFeatureBaseTemplate 灵兽榜
local UIRankServantFeatureTemplate = {}

setmetatable(UIRankServantFeatureTemplate, luaclass.UIRankFeatureBaseTemplate)

function UIRankServantFeatureTemplate:SetTemplate()
...
function UIRankServantFeatureTemplate:SetServantList()
...
function UIRankServantFeatureTemplate:TrySetPos()
    self:RunBaseFunction("TrySetPos")

    if self.unitTbl:GetServantGrid() ~= nil then
        local origionPos = self.unitTbl:GetServantGrid().transform.localPosition
        local posx = CS.Cfg_RankTableManager.Instance:GetComponentPos(self.unitTbl.rankConfigId, LuaRankComponentSystem.servant)
...
--是否需要显示灵兽列表5
function UIRankServantFeatureTemplate:IsShowServantList()
...
return UIRankServantFeatureTemplate
