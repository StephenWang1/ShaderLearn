...
---待分发消息列表
ClientMsgManager.DelayMsgIDList = {}
---分发中的消息列表
ClientMsgManager.DelayMsgSendingList = {}
...
---延迟分发消息
---@param msgID number
function ClientMsgManager.DoDelayCallBack(msgID)
    for i = 1, #ClientMsgManager.DelayMsgIDList do
        if ClientMsgManager.DelayMsgIDList[i] == msgID then
            return
        end
    end
    table.insert(ClientMsgManager.DelayMsgIDList, msgID)
end
...
function ClientMsgManager.OnUpdate()
    if #ClientMsgManager.DelayMsgSendingList > 0 then
        for i = #ClientMsgManager.DelayMsgSendingList, 1, -1 do
            table.remove(ClientMsgManager.DelayMsgSendingList, i)
        end
    end
    if #ClientMsgManager.DelayMsgIDList > 0 then
        for i = 1, #ClientMsgManager.DelayMsgIDList do
            table.insert(ClientMsgManager.DelayMsgSendingList, ClientMsgManager.DelayMsgIDList[i])
        end
        for i = #ClientMsgManager.DelayMsgIDList, 1, -1 do
            table.remove(ClientMsgManager.DelayMsgIDList, i)
        end
        for i = 1, #ClientMsgManager.DelayMsgSendingList do
            ClientMsgManager.IsDoingCallback = true
            ClientMsgManager.msgID = ClientMsgManager.DelayMsgSendingList[i]
            ClientMsgManager.data = nil
            try(ClientMsgManager.DoCallbackTryTable)
            ClientMsgManager.msgID = nil
            ClientMsgManager.data = nil
